...
module.exports = {
    ...
    if (!args.length) return message.reply(" **Tienes que mencionar a alguien... **")
    
    const usuario = message.mentions.users.first()

    const embed = new Discord.MessageEmbed()
    .setTitle(`${message.author.tag} asesino a  ${usuario.tag} `)
    .setImage(`${kill}`)
    .setFooter({text: `Dejen su F por ${usuario.tag} :'v`})
    .setColor("RANDOM")
